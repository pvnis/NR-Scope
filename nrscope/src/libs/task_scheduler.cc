...
int TaskSchedulerNRScope::PrewarmWorkers()
{
  std::cout << "Pre-warming " << nof_workers << " worker(s)..." << std::endl;
  for (uint32_t i = 0; i < nof_workers; i++) {
    if (workers[i]->PrewarmDecoders(&task_scheduler_state) < SRSRAN_SUCCESS) {
      ERROR("Worker %d failed to pre-warm its decoders", i);
      return NR_FAILURE;
    }
  }
  std::cout << "Workers pre-warmed." << std::endl;
  return SRSRAN_SUCCESS;
}
